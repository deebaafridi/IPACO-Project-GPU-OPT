 ...
 */
 ...
 __global__ void mm2_kernel1(int ni, int nj, int nk, int nl, DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE *tmp, DATA_TYPE *A, DATA_TYPE *B)
 {
    ...
    int idx = row * nj + col;
     ...
     else
     {
         ...
         tmp[idx] = static_cast<float>(product_d); // Cast result back to float before storing
     }
     __syncwarp();
 ...
 __global__ void mm2_kernel2(int ni, int nj, int nk, int nl,
                             DATA_TYPE alpha, DATA_TYPE beta,
                             DATA_TYPE *tmp, DATA_TYPE *C, DATA_TYPE *D)
 {
    //Declaring a varibales in shared memory which are common for a Threadblock
     __shared__ int fp32CountInSM;
     __shared__ int fp64CountInSM;
    ...
    int idx = row * nl + col;
     ...
     if (warpFP32Choice)  // Perform matrix multiplication using either FP32 or FP64 units based on warp decision
     {
         for (int k = 0; k < nj; ++k)
         {
             float t = tmp[row * nj + k];
             float c = C[k * nl + col];
             product_f += t * c;
         }
         D[idx] = beta * D[idx] + product_f;
     }
     else
     {
         for (int k = 0; k < nj; ++k)
         {
             double t = (double)tmp[row * nj + k];  //cast data to double so FP64 units are utilized
             double c = (double)C[k * nl + col];
             product_d += t * c;
         }
         D[idx] = beta * D[idx] + static_cast<float>(product_d);  // Cast result back to float before storing
     }
     __syncwarp();
