                                             ...
                                             DATA_TYPE *a, DATA_TYPE *b, DATA_TYPE *c)
{
    ...
    if (row < ni && col < nj) {
        if (useFP64) {
            double c_val_fp64 = (double)c[row * nj + col];
            c_val_fp64 *= (double)beta;

            for (int k_idx = 0; k_idx < nk; k_idx++) {
                double a_val = (double)a[row * nk + k_idx];
                double b_val = (double)b[k_idx * nj + col];
                c_val_fp64 += (double)alpha * a_val * b_val;
            }

            c[row * nj + col] = (DATA_TYPE)c_val_fp64;
        } else {
            float c_val_fp32 = (float)c[row * nj + col]; 
            c_val_fp32 *= (float)beta;

            for (int k_idx = 0; k_idx < nk; k_idx++) {
                float a_val = (float)a[row * nk + k_idx];
                float b_val = (float)b[k_idx * nj + col];
                c_val_fp32 += (float)alpha * a_val * b_val;
            }

            c[row * nj + col] = (DATA_TYPE)c_val_fp32;
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    init(ni, nj, nk, &alpha_val, &beta_val, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(C));

    GPU_argv_init();

    gemmCuda(ni, nj, nk, alpha_val, beta_val, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(C), POLYBENCH_ARRAY(C_outputFromGpu));

#ifdef RUN_ON_CPU
   
    polybench_start_instruments;

    gemm(ni, nj, nk, alpha_val, beta_val, POLYBENCH_ARRAY(A), POLYBENCH_ARRAY(B), POLYBENCH_ARRAY(C));
